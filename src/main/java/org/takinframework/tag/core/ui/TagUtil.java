...
public class TagUtil {
	 ...
	 * 手工拼接JSON
	 */
	public static String getComboBoxJson(List<SysRole> list, List<SysRole> roles) {
		StringBuffer buffer = new StringBuffer();
		buffer.append("[");
		for (SysRole node : list) {
			if (roles.size() > 0) {
				buffer.append("{\"id\":" + node.getId() + ",\"text\":\"" + node.getRoleName() + "\"");
				for (SysRole node1 : roles) {
