...
<div class="main" style="background:none;">
   <div class="main_top">
     <ul>
       <li><a href="tfBarnController.do?tfLiaqin&id=${tfBarn.id}" id="l_1"  onfocus="this.blur();" class="current">粮情检查记录</a></li>
   ...
   <div class="m_left">
   <iframe id="searchFrame" name="searchFrame" data-url="tfLiaqinController.do?tfLiaqin&tfBarnId=${tfBarn.id}" src="tfLiaqinController.do?tfLiaqin&tfBarnId=${tfBarn.id}" frameborder="0" scrolling="no" height="580px" width="160px"></iframe>
   </div><!--m_left e-->
   <div class="m_right">
   <iframe id="detailFrame" name="detailFrame" data-url="tfLiaqinController.do?tfLiaqinAddOrUpdate&tfBarnId=${tfBarn.id}" src="tfLiaqinController.do?tfLiaqinAddOrUpdate&tfBarnId=${tfBarn.id}" frameborder="0" scrolling="no" height="570px" width="740px"></iframe>
   </div><!--m_right e-->
   </div><!--main_center e-->
</div><!--main e-->
	...
	$(function(){
		 ...
		 */
		 ...
		 .on('click','a.dc',function(){
			...
			if(id != undefined){
				$('#exportFrom').attr('action','tfLiaqinController.do?export&id='+id);
				$('#exportFrom').submit();
			}else{
				W.$.dialog({
        			id:'alert',
